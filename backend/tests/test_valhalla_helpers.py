...
class FakeSession:
    """Mimic AsyncSession.execute() returning FakeResult for provided items."""

    def __init__(self, items):
        self._items = items

    async def execute(self, *_, **__):
        # We ignore the incoming query object; we just return the items we were given.
...
async def test_itinerary_happy_path(monkeypatch):
    ...
    # Deterministic embedding for ordering
    ...
    # Async route mock returning a valid Trip
